...
typedef NS_ENUM (NSInteger, ToolBarTag){
    ToolBarTagBlack = 100,
    ToolBarTagRed = 101,
    ToolBarTagBlue = 102,
    ToolBarTagBold = 200,
    ToolBarTagItalic = 201,
    ToolBarTagUnderline = 202,
    ToolBarTagAlignLeft = 500,
    ToolBarTagAlignCenter = 501,
    ToolBarTagAlignRight = 502,
};

static NSInteger currentFontSize;
...
- (void)createToolBarButton{
    ...
    [button setFrame:CGRectMake(15, 5, 50, 34)];
    ...
    [button setBackgroundColor:[UIColor colorWithRed:0x9a/255.0 green:0xcc/255.0 blue:0xfd/255.0 alpha:1]];
    [self.toolbarHolder addSubview:button];
    
    
    CGFloat width = 35;
    
    CGFloat begin = self.view.frame.size.width - (width + 10) * 6;
    
     NSArray<UIColor *> *array = [[NSArray<UIColor *> alloc] initWithObjects:
                                  [UIColor blackColor],
                                  [UIColor redColor],
                                  [UIColor blueColor],nil];
    
    for (int i = 0; i < 3; i ++){
        UIButton *button = [self createToolBarButtonWithTag:100 + i andLeft:begin andTop:5 andWidth:20 andSel:@selector(changeTextColor:)];
        [button setBackgroundColor:[array objectAtIndex:i]];
        
        begin = begin + 45;
    }
    
    NSArray<NSString *> *fontStyleNameArray = [[NSArray<NSString *> alloc] initWithObjects:@"ZSSbold",@"ZSSitalic",@"ZSSunderline",nil];
    
    for (int i = 0; i < 3; i ++){
        UIButton *button = [self createToolBarButtonWithTag:200 + i andLeft:begin andTop:5 andWidth:20 andSel:@selector(changeFontStyle:)];
        [button setBackgroundImage:[UIImage imageNamed:[fontStyleNameArray objectAtIndex:i]] forState:UIControlStateNormal];
        begin = begin + 45;
    }
    
    begin = 15;
    NSArray<NSString *> *doNameArray = [[NSArray<NSString *> alloc] initWithObjects:@"ZSSundo",@"ZSSredo",nil];
    for (int i = 0; i < 2; i ++){
        UIButton *button = [self createToolBarButtonWithTag:300 + i andLeft:begin andTop:49 andWidth:35 andSel:@selector(doFun:)];
        [button setBackgroundImage:[UIImage imageNamed:[doNameArray objectAtIndex:i]] forState:UIControlStateNormal];
        begin = begin + 45;
    }
    
    begin = self.view.frame.size.width - (width + 10) * 6;
    NSArray<NSString *> *fontSizeArray = [[NSArray<NSString *> alloc] initWithObjects:@"ZSSh1",@"ZSSh2",nil];
    for (int i = 0; i < 2; i ++){
        UIButton *button = [self createToolBarButtonWithTag:400 + i andLeft:begin andTop:49 andWidth:35 andSel:@selector(changeFontSize:)];
        [button setBackgroundImage:[UIImage imageNamed:[fontSizeArray objectAtIndex:i]] forState:UIControlStateNormal];
        begin = begin + 45;
    }
    
    NSArray<NSString *> *textAlignArray = [[NSArray<NSString *> alloc] initWithObjects:@"ZSSleftjustify",@"ZSScenterjustify",@"ZSSrightjustify",nil];
    for (int i = 0; i < 3; i ++){
        UIButton *button = [self createToolBarButtonWithTag:500 + i andLeft:begin andTop:49 andWidth:20 andSel:@selector(changeTextAlign:)];
        [button setBackgroundImage:[UIImage imageNamed:[textAlignArray objectAtIndex:i]] forState:UIControlStateNormal];
        begin = begin + 45;
    }
    
    UIButton *hideKeyboard = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    hideKeyboard.clipsToBounds = YES;
    [hideKeyboard setFrame:CGRectMake(begin, 49, 35, 35)];
    [hideKeyboard addTarget:self action:@selector(dismissKeyboard) forControlEvents:UIControlEventTouchUpInside];
    [hideKeyboard setBackgroundImage:[UIImage imageNamed:@"ZSSkeyboard"] forState:UIControlStateNormal];
    [self.toolbarHolder addSubview:hideKeyboard];
}

- (UIButton *)createToolBarButtonWithTag:(NSInteger)tag andLeft:(CGFloat)left andTop:(CGFloat)top andWidth:(CGFloat)width andSel:(nonnull SEL)sel{
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(left, top, 35, 35)];
    ...
    UIButton *button = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    button.clipsToBounds = YES;
    button.frame = CGRectMake((35 - width)/2, (35 - width)/2, width, width);
...
- (void)changeTextColor:(UIButton *)button{
    [self.editorView stringByEvaluatingJavaScriptFromString:@"zss_editor.prepareInsert();"];
    [self setSelectedColor:button.backgroundColor tag:1];
}

- (void)changeFontStyle:(UIButton *)button{
...
- (void)doFun:(UIButton *)button{
    if (button.superview.tag == 300){
        [self undo:nil];
    }else{
        [self redo:nil];
    }
...
- (void)changeFontSize:(UIButton *)button{
    if (button.superview.tag == 400){
        if (currentFontSize == 0){
            return;
        }
        currentFontSize = currentFontSize < 6 ? currentFontSize + 1 : 0;
    }else{
        if (currentFontSize == 1){
            return;
        }
        currentFontSize = currentFontSize == 0 ? 6 : currentFontSize - 1;
    }
    NSString *trigger;
    if (currentFontSize == 0){
        trigger = @"zss_editor.setHeading('h6');";
    }else{
        trigger = [NSString stringWithFormat:@"zss_editor.setHeading('h%zd');",currentFontSize];
    }
    [self.editorView stringByEvaluatingJavaScriptFromString:trigger];
}

- (void)changeTextAlign:(UIButton *)button{
...
- (void)updateToolBarWithButtonName:(NSString *)name {
    ...
    NSString *rgb = @"rgb+.*\\)";
    NSError *error;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:rgb options:NSRegularExpressionCaseInsensitive error:&error];
    NSTextCheckingResult *result = [regex firstMatchInString:name options:0 range:NSMakeRange(0, [name length])];
    NSString *hexRgb = nil;
    if (result){
        NSString *rgbStr = [name substringWithRange:result.range];
        name = [name stringByReplacingOccurrencesOfString:rgbStr withString:@""];
        rgbStr = [rgbStr stringByReplacingOccurrencesOfString:@"rgb(" withString:@""];
        rgbStr = [rgbStr stringByReplacingOccurrencesOfString:@" " withString:@""];
        rgbStr = [rgbStr stringByReplacingOccurrencesOfString:@")" withString:@""];
        NSArray<NSString *> *rgbArray = [rgbStr componentsSeparatedByString:@","];
        if (rgbArray.count == 3){
            hexRgb = [NSString stringWithFormat:@"#%02X%02X%02X",[rgbArray[0] intValue],[rgbArray[1] intValue],[rgbArray[2] intValue]];
        }
    }
    
    NSMutableArray *itemNames = [NSMutableArray new];
    [itemNames addObjectsFromArray:[name componentsSeparatedByString:@","]];
    if (hexRgb){
        [itemNames addObject:hexRgb];
    }
    ...
    itemNames = [NSMutableArray arrayWithArray:itemsModified];
    
    self.editorItemsEnabled = itemNames;
    
    // Highlight items

    [self setView:[self.view viewWithTag:ToolBarTagBold] backgroundColor:![itemNames containsObject:@"bold"]];
    [self setView:[self.view viewWithTag:ToolBarTagItalic] backgroundColor:![itemNames containsObject:@"italic"]];
    [self setView:[self.view viewWithTag:ToolBarTagUnderline] backgroundColor:![itemNames containsObject:@"underline"]];
    [self setView:[self.view viewWithTag:ToolBarTagBlack] backgroundColor:![itemNames containsObject:@"#000000"]];
    [self setView:[self.view viewWithTag:ToolBarTagRed] backgroundColor:![itemNames containsObject:@"#FF0000"]];
    [self setView:[self.view viewWithTag:ToolBarTagBlue] backgroundColor:![itemNames containsObject:@"#0000FF"]];
    [self setView:[self.view viewWithTag:ToolBarTagAlignLeft] backgroundColor:![itemNames containsObject:@"justifyLeft"]];
    [self setView:[self.view viewWithTag:ToolBarTagAlignCenter] backgroundColor:![itemNames containsObject:@"justifyCenter"]];
    [self setView:[self.view viewWithTag:ToolBarTagAlignRight] backgroundColor:![itemNames containsObject:@"justifyRight"]];
    if ([itemNames containsObject:@"h1"]){
        currentFontSize = 1;
    }else if ([itemNames containsObject:@"h2"]){
        currentFontSize = 2;
    }else if ([itemNames containsObject:@"h3"]){
        currentFontSize = 3;
    }else if ([itemNames containsObject:@"h4"]){
        currentFontSize = 4;
    }else if ([itemNames containsObject:@"h5"]){
        currentFontSize = 5;
    }else if ([itemNames containsObject:@"h6"]){
        currentFontSize = 6;
    }else{
        currentFontSize = 0;
    }
