...
//            获取屏幕的高度
            ...
            storage.getItem('gpsData', e => {
                if (!utils.isNull(e.data)) {
                    var addressLng = JSON.parse(e.data)
                    _this.lng = addressLng.lng;
                    _this.lat = addressLng.lat;
                    _this.address = addressLng.address
                    _this.getAllArticle();
                    _this.$emit("addressChange",_this.address)
                }else {
                    _this.getAllArticle();
                }
                _this.getGps();
        ...
        methods:{
            ...
            getGps(){
                ...
                event.getLocation(function (e) {
                    if (e.type=='success') {
                        _this.lng = e.data.lng;
                        _this.lat = e.data.lat;
                        _this.address=e.data.district
                        _this.getAllArticle();
                        _this.$emit("addressChange",_this.address)
                        var E = {
                            lng: e.data.lng,
                            lat: e.data.lat,
                            address:e.data.district,
                        };
                        E = JSON.stringify(E);
                        storage.setItem('gpsData', E, function (e) {
                        });
                    }
                    else {
...
//            获取文章列表
            getAllArticle(){
                let _this = this;
                GET('weex/article/lbs.jhtml?pageStart=' + this.pageStart + '&pageSize=' + this.pageSize+'&lng='+_this.lng+'&lat='+_this.lat,function (data) {
