    ...
    <div class="wrapper">
        ...
        <div class="bottomTotal" @swipe="onpanmove($event,index)" @touchstart="onToptouchstart($event)" v-if="summarylist.length>0">
            ...
            <div class="totalCell">
                ...
                <text class="money">¥{{summarylist[0].subTotal | currencyfmt}}</text>
            </div>
...
//            判断水站是否重复,并且在最后一个显示统计
            ...
            linkToDetail(sellerId){
                ...
                this.clicked = true;
                var _this = this;
                event.openURL(utils.locate('view/shop/report/sendSingle_detail.js?sellerId='+sellerId+'&beginTime='+encodeURIComponent(this.beginTime)+'&endTime='+encodeURIComponent(this.endTime)), function (data) {
                    _this.clicked = false;
                })
            }
        },
