            ...
            <cell v-for="friend in friendsList" ref="listref" >
                ...
                <div v-for="(item,index) in friend.name">
                    <div class="addFriendsBorder" >
                        ...
                        <div class="friendsLine" @click="goChat(item.id)"  @touchmove="onpanmove($event,index)" @touchend="onpanend($event,index)">
...
//        ==============================
            ontouchmove:function(count,e){//按住字母导航栏并拖动时触发
                if(this.pressPoint == - 1){//记录每次第一次滑动按压的点
                    this.pressPoint = e.changedTouches[0].pageX;
                }else{
                    this.movePoint = this.pressPoint - e.changedTouches[0].pageX;//求移动的距离(绝对值)
                    if(this.movePoint > 0){
                        var _this = this;
                        if(e.currentTarget == null || e.currentTarget == '' || e.currentTarget == 'undefinded' ){
                        }else{
                            animation.transition(e.currentTarget, {
                                styles: {
                                    transform: `translateX(${this.movePoint})`,
                                },
                                duration: 350, //ms
...
//                获取当前点击的元素。
            ...
            onpanend:function(e,index){
                let endPoint = Math.abs(this.pressPoint) - Math.abs(e.changedTouches[0].pageX);//求移动的距离(绝对值)
                    var _this = this;
                    if(e.currentTarget == null || e.currentTarget == '' || e.currentTarget == 'undefinded' ){
                    }else{
                        if(endPoint >= 65){
...
//                获取当前点击的元素。
                var _this = this;
                    ...
                    if(this.pressPoint == - 1){//记录每次第一次滑动按压的点
                        ...
                        this.movePoint = Math.abs(this.pressPoint) - Math.abs(e.changedTouches[0].pageX);//求移动的距离(绝对值)
                  modal.toast({message:e.changedTouches[0].pageX,duration:0.3});
                        if(this.movePoint >= 0){
                            var _this = this;
                            if(e.currentTarget == null || e.currentTarget == '' || e.currentTarget == 'undefinded' ){
                            }else{
                                if(this.movePoint >= 130){
                                    animation.transition(e.currentTarget, {
                                        styles: {
                                            transform: `translateX(-130)`,
                                        },
